void calo_occupancy_endcap(TString input_file){

TFile* f_Bkg = new TFile(input_file);

TTree *t_Ecal  = (TTree*) f_Bkg->Get("ECalEndcapCollectionTuple");
TTree *t_Hcal = (TTree*) f_Bkg->Get("HCalEndcapCollectionTuple");
...
for (Int_t i_entry=0; i_entry < nentries_Ecal; i_entry++){
  ...
  for (Int_t j=0; j<nsch; j++){
    ...
    if (scene[j]*1000>e_th_Ecal) h_occ_Ecal->Fill(abs(z),1./Area_Ecal); //divided by the module area as a weight
...
for (Int_t i_entry=0; i_entry < nentries_Hcal; i_entry++){
  ...
  for (Int_t j=0; j<nsch; j++){
    ...
    if (scene[j]*1000>e_th_Hcal) h_occ_Hcal->Fill(abs(z),1./Area_Hcal); //divided by the module area as a weight
